...
## Discussion
With our app, we provide the audience with sufficient information to make a wise choice for their flights to mitigate the chance of a delayed flights. From the analysis of different factors we managed to draw insights and analyze the reasons behind the flight delays.

### 1. Airline company matters to flight delay
Alaska Airlines, Southwest Airlines, and Delta Airlines are the best 3 performers. Some airlines like Skywest Airlines and JetBlue Airlines do need to pay more attention to their flight delay issues. Choosing a flight carrier wisely allow the potential travelers save time from the potential flight delay.
...
It is interesting to observe that the traveling distance is not a significant factor that impact the flight delay. Flights usually delay around 30 minutes regardless of the travel distance. Surprisingly, long flight distance usually does not have a longer delay time. A plausible explaination behind this observation could be that the flights can fly faster to make up for the lost time and mitigate the arrival delay.


### 4. Peace in mind when flying ‘In-N-Out’ on the West Coast
The departure and arrival locations usually have a strong impact for flight delays. Departure and arrival flights in North Dakota are very likely to be delayed for a longer period of time based on the historical data. On the other hand, flights arriving and departing on the West Coast are more likely to be on time. The airport facilities on the West Coast, such as in Seattle, San Fransico, and Los Angeles are more advanced and can handle the traffic better than a state like North Dakota.


### 5. Watch out for your timing and your carrier
When combining the flighting time and carrier, a more clear picture is drawn for the audience. Comparing United Airlines with Delta Airlines (with a similar number of flights), we can see United Airlines is more likely to have longer delays than Delta. Longer delays (350-400min) happen more frequently in December compared to the other months, which might be caused by the holiday season. The shorter and medium delays (50-300min) happen more frequently in June-August, which might indicate that people travel more often in the summertime. There are fewer flights from January-April, which might indicate that fewer people choose to travel during this time.


### 6. Worry less when traveling between big hubs
When looking the number of fights and average delay time among different airports on the map, we found that the average delay time of flights between two big hubs is shorter than that of flights from a big hub to a smaller airport. A rational hypothesis is that the airports prefer prioritizing flights between two big hubs. In addition, smaller airports usually only connect to a few big hubs and are likely to have longer average delay times. Therefore, the travelers should choose flights between big hubs whenever possible to avoid potential flight delay.


## Future Work

We utilize a linear regression model to predict the delay time based on the selected features. To refine the app prediction accuracy, we could explore different machine learning models, such as decision tree and neural network to improve the accuray of the prediction. The current model is trained with the whole dataset. Since cases may vary between airlines, the prediction might be more accurate if we could develop models for each airline separately. To extend the accessibility of the app, we could consider to embed this app into some exsiting platforms, such as google flights. Moreover, we could consider develop a mobile version to ensure users can easily access it during the traveling.
